...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::SpiInit(void)
{
...
/****************************************************************
*FUNCTION NAME:SpiDeInit
*FUNCTION     :spi communication disable
*INPUT        :none
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::SpiDeInit(void)
{
  // disable SPI
  SPI.endTransaction();
  SPI.end();
...
*OUTPUT       : none
****************************************************************/
void CC1101_DRIVER::GDO_Set (void)
{
	pinMode(GDO0, OUTPUT);
	pinMode(GDO2, INPUT);
}
...
*OUTPUT       : none
****************************************************************/
void CC1101_DRIVER::GDO0_Set (void)
{
  pinMode(GDO0, INPUT);
}
/****************************************************************
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::SpiWriteReg(byte addr, byte value)
{
  //SpiInit();
  digitalWrite(SS_PIN, LOW);
  while(digitalRead(MISO_PIN));
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::SpiWriteBurstReg(byte addr, byte *buffer, byte num)
{
  byte i, temp;
  //SpiInit();
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::SpiStrobe(byte strobe)
{
  //SpiInit();
  digitalWrite(SS_PIN, LOW);
  while(digitalRead(MISO_PIN));
...
****************************************************************/
byte CC1101_DRIVER::SpiReadReg(byte addr) 
{
  byte temp, value;
  //SpiInit();
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::SpiReadBurstReg(byte addr, byte *buffer, byte num)
{
  byte i,temp;
  //SpiInit();
...
*OUTPUT       :status value
****************************************************************/
byte CC1101_DRIVER::SpiReadStatus(byte addr) 
{
  byte value,temp;
  //SpiInit();
  temp = addr | READ_BURST;
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setSpiPin(byte sck, byte miso, byte mosi, byte ss){
  SCK_PIN = sck;
  MISO_PIN = miso;
  MOSI_PIN = mosi;
  SS_PIN = ss;
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setGDO(byte gdo0, byte gdo2){
GDO0 = gdo0;
GDO2 = gdo2;  
GDO_Set();
}
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setGDO0(byte gdo0){
GDO0 = gdo0;
GDO0_Set();
}
/****************************************************************
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setCCMode(bool s){
ccmode = s;
if (ccmode == 1){
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setModulation(byte m){
if (m>4){m=4;}
modulation = m;
Split_MDMCFG2();
switch (m)
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setPA(int p)
{
int a=0;
pa = p;
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setMHZ(float mhz){
byte freq2 = 0;
byte freq1 = 0;
uint16_t freq0 = 0;
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::Calibrate(void){
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setClb(byte b, byte s, byte e){
if (b == 1){
clb1[0]=s;
clb1[1]=e;  
}
...
*OUTPUT       :none
****************************************************************/
byte CC1101_DRIVER::getMode(void){
return trxstate;
}
/****************************************************************
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setSyncWord(byte sh, byte sl){
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setAddr(byte v){
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setPQT(byte v){
Split_PKTCTRL1();
pc1PQT = 0;
if (v>7){v=7;}
pc1PQT = v*32;
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setCRC_AF(bool v){
Split_PKTCTRL1();
pc1CRC_AF = 0;
if (v==1){pc1CRC_AF=8;}
SpiWriteReg(CC1101_PKTCTRL1, pc1PQT+pc1CRC_AF+pc1APP_ST+pc1ADRCHK);
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setAppendStatus(bool v){
Split_PKTCTRL1();
pc1APP_ST = 0;
if (v==1){pc1APP_ST=4;}
SpiWriteReg(CC1101_PKTCTRL1, pc1PQT+pc1CRC_AF+pc1APP_ST+pc1ADRCHK);
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setAdrChk(byte v){
Split_PKTCTRL1();
pc1ADRCHK = 0;
if (v>3){v=3;}
pc1ADRCHK = v;
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setWhiteData(bool v){
Split_PKTCTRL0();
pc0WDATA = 0;
if (v == 1){pc0WDATA=64;}
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setPktFormat(byte v){
Split_PKTCTRL0();
pc0PktForm = 0;
if (v>3){v=3;}
pc0PktForm = v*16;
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setCrc(bool v){
Split_PKTCTRL0();
pc0CRC_EN = 0;
if (v==1){pc0CRC_EN=4;}
SpiWriteReg(CC1101_PKTCTRL0, pc0WDATA+pc0PktForm+pc0CRC_EN+pc0LenConf);
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setLengthConfig(byte v){
Split_PKTCTRL0();
pc0LenConf = 0;
if (v>3){v=3;}
pc0LenConf = v;
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setPacketLength(byte v){
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setDcFilterOff(bool v){
Split_MDMCFG2();
m2DCOFF = 0;
if (v==1){m2DCOFF=128;}
SpiWriteReg(CC1101_MDMCFG2, m2DCOFF+m2MODFM+m2MANCH+m2SYNCM);
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setManchester(bool v){
Split_MDMCFG2();
m2MANCH = 0;
if (v==1){m2MANCH=8;}
SpiWriteReg(CC1101_MDMCFG2, m2DCOFF+m2MODFM+m2MANCH+m2SYNCM);
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setSyncMode(byte v){
Split_MDMCFG2();
m2SYNCM = 0;
if (v>7){v=7;}
m2SYNCM=v;
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setFEC(bool v){
Split_MDMCFG1();
m1FEC=0;
if (v==1){m1FEC=128;}
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setPRE(byte v){
Split_MDMCFG1();
m1PRE=0;
if (v>7){v=7;}
m1PRE = v*16;
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setChannel(byte ch){
chan = ch;
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setChsp(float f){
Split_MDMCFG1();
byte MDMCFG0 = 0;
m1CHSP = 0;
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setRxBW(float f){
Split_MDMCFG4();
int s1 = 3;
int s2 = 3;
for (int i = 0; i<3; i++){
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setDRate(float d){
Split_MDMCFG4();
float c = d;
byte MDMCFG3 = 0;
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setDeviation(float d){
float f = 1.586914;
float v = 0.19836425;
int c = 0;
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::Split_PKTCTRL1(void){
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::Split_PKTCTRL0(void){
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::Split_MDMCFG1(void){
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::Split_MDMCFG2(void){
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::Split_MDMCFG4(void){
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::RegConfigSettings(void) 
{   
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::SetTx(float mhz)
{
  SpiStrobe(CC1101_SIDLE);
  setMHZ(mhz);
  SpiStrobe(CC1101_STX);        //start send
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::SetRx(float mhz)
{
  SpiStrobe(CC1101_SIDLE);
  setMHZ(mhz);
  SpiStrobe(CC1101_SRX);        //start receive
...
*OUTPUT       :none
****************************************************************/
int CC1101_DRIVER::getRssi(void)
{
int rssi;
...
*OUTPUT       :none
****************************************************************/
byte CC1101_DRIVER::getLqi(void)
{
byte lqi;
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setSres(void)
{
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::setSidle(void)
{
  SpiStrobe(CC1101_SIDLE);
  trxstate=0;
}
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::goSleep(void){
  trxstate=0;
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::SendData(char *txchar)
{
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::SendData(byte *txBuffer,byte size)
{
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::SendData(char *txchar,int t)
{
int len = strlen(txchar);
byte chartobyte[len];
for (int i = 0; i<len; i++){chartobyte[i] = txchar[i];}
...
*OUTPUT       :none
****************************************************************/
void CC1101_DRIVER::SendData(byte *txBuffer,byte size,int t)
{
  SpiWriteReg(CC1101_TXFIFO,size);
  SpiWriteBurstReg(CC1101_TXFIFO,txBuffer,size);      //write data to send
  SpiStrobe(CC1101_SIDLE);
...
*OUTPUT       :none
****************************************************************/
bool CC1101_DRIVER::CheckCRC(void){
...
****************************************************************/
bool CC1101_DRIVER::CheckRxFifo(int t){
if(trxstate!=2){SetRx();}
...
*OUTPUT       :flag: 0 no data; 1 receive data 
****************************************************************/
byte CC1101_DRIVER::CheckReceiveFlag(void)
{
  if(trxstate!=2){SetRx();}
...
****************************************************************/
byte CC1101_DRIVER::ReceiveData(byte *rxBuffer)
{
	byte size;
	byte status[2];
